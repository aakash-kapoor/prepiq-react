...
function getDifficultyClasses(difficulty: string): string {
    ...
    if (normalized.includes('senior')) return 'bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 border-red-200 dark:border-red-800';
    if (normalized.includes('junior')) return 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800';
...
export default function ResultsPanel({ analysisResult, isLoading, isSaving, panelRef, onSave }: ResultsPanelProps) {
    return (
        <div
            ref={panelRef}
            className="flex-1 bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-6 shadow-sm flex flex-col justify-between min-h-[200px] lg:min-h-[500px] scroll-mt-20 md:scroll-mt-5"
        >
            {isLoading ? (
                ...
                <div className="space-y-6 h-full flex flex-col justify-between">
                    <div className="space-y-5">
                        <div className="border-b border-gray-100 dark:border-slate-700 pb-3 flex flex-wrap justify-between items-start gap-2">
                            <div className="min-w-0 flex-1">
                                <span className="text-xs font-bold text-emerald-600 dark:text-emerald-500 flex items-center gap-1">
                                    Analysis Complete ✓
                                </span>
                                <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 tracking-tight mt-0.5 break-words">{analysisResult.roleTitle || 'Ionic Angular Developer'}</h3>
                                <p className="text-[11px] text-slate-400 dark:text-slate-500 font-semibold tracking-wide break-words">Targeting: {analysisResult.companyType || 'Generic Tech Company/Agency'}</p>
                            </div>
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className="flex flex-wrap gap-1.5">
                                        {analysisResult.extractedSkills.filter((item: any) => item.category === 'Core').map((item: any, i: number) => (
                                            <span key={i} className="text-xs px-2.5 py-1 rounded-lg font-medium border bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 border-indigo-100 dark:border-indigo-800">
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        {analysisResult.redFlags.map((flag: string, i: number) => (
                                            <span key={i} className="text-xs px-3 py-2 rounded-xl font-medium border bg-rose-50 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300 border-rose-100 dark:border-rose-800 flex items-start gap-2 leading-relaxed">
                        ...
                        <div className="space-y-2">
                            <h4 className="text-[11px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Top Interview Target Focus</h4>
                            <ol className="space-y-2 text-xs text-slate-600 dark:text-slate-300 font-medium">
                                {analysisResult.focusAreas?.map((area: string, i: number) => (
                                    <li key={i} className="flex items-start gap-2.5 leading-normal">
                                        <span className="text-slate-300 dark:text-slate-600 font-extrabold select-none mt-0.5">{i + 1}</span>
                                        <span>{area}</span>
                                    </li>
                                ))}
                            </ol>
